...
class Timeshift {
    ...
    public function getInputForms($action){
        ...
        $this->clientCrtl = $this->select->getClientSelectCtrl($this->job_owner);
        $this->payrollEmplCrtl = $this->select->getPayrollEmplSelectCtrl($this->payrollEmpl);
    ?>
   <html>
  <head>
    <meta charset="utf-8">
          ...
          <section class="content">
            <div class="box box-success">
              <div class="box-header with-border">
                ...
                <button type = "button" class="btn btn-primary pull-right" style = '' onclick = "window.location.href='timeshift.php'">Search</button>
                <?php if(getWindowPermission($_SESSION['m'][$_SESSION['empl_id']],'create')){?>
                <button type = "button" class="btn btn-primary pull-right" style = 'margin-right:10px;' onclick = "window.location.href='timeshift.php?action=createForm'">Create New</button>
                <?php }?>
              </div>
                
                <form id = 'applicant_form' class="form-horizontal" action = 'timeshift.php?action=create' method = "POST" enctype="multipart/form-data">
        ...
        $("#applicant_form").validate({
                  rules: 
                  {
                      ...
                      applicant_login_email:
                      {
                          required: true,
                          remote: {
                                  url: "timeshift.php?action=validate_email",
                                  type: "post",
                                  data: 
                                        {
                    ...
                    success: function(data) {
                        ...
                        if(jsonObj['department'] != null){                 
                           ...
                           for(var i=0;i<jsonObj['department'].length; i++){
                                $('#department_name').append(jsonObj['department'][i]);
                           }
                        }
                        else {
                            $('#department_name').empty();
                        ...
                        if(jsonObj['applicant'] != null){
                           ...
                           for(var i=0;i<jsonObj['applicant'].length; i++){
                                $('#applicant_name').append(jsonObj['applicant'][i]);
                           }
                        }
                        else {
                            $('#applicant_name').empty();
                        ...
                        if(jsonObj['job'] != null){  
                           ...
                           for(var i=0;i<jsonObj['job'].length; i++){
                                $('#job_title').append(jsonObj['job'][i]);
                           }
                        }
                        else {
                            $('#job_title').empty();
                ...
                $.ajax({ 
                    ...
                    success: function(data) {
                        ...
                        if(jsonObj['timeshift'] != null){
                           ...
                           document.getElementById("start_time").value = jsonObj['timeshift']['timeshift_start_time'];
                           document.getElementById("end_time").value = jsonObj['timeshift']['timeshift_end_time'];
                           document.getElementById("ot_rate").value = jsonObj['timeshift']['timeshift_ot_rate'];
                           document.getElementById("working_day").value = jsonObj['timeshift']['timeshift_work_day'];
                           }
                        else {
            ...
            <div class="form-group">                
                ...
                <div class="col-sm-3">
                     <select class="form-control select2" id="job_type" name="job_type" style = 'width:100%'>
                        <option value="PD" <?php if($this->job_status == 'PD'){ echo 'SELECTED';}?>>Part Time (Daily)</option>
                        <option value="PH" <?php if($this->job_status == 'PH'){ echo 'SELECTED';}?>>Part Time (Hourly)</option>
                        <option value="F" <?php if($this->job_status == 'F'){ echo 'SELECTED';}?>>Full Time</option>
                        <option value="C" <?php if($this->job_status == 'C'){ echo 'SELECTED';}?>>Contract</option>
                     </select>
                </div>
            </div>    
                <div class="fullncontract">
                    <h3><u>Company Time Shift</u></h3>
                    <div class="form-group">
                          <label for="company_name" class="col-sm-2 control-label">Company Name</label>
                          <div class="col-sm-3">
                                 <select class="form-control select2" id="company_name" name="company_name" style = 'width:100%'>
                                     <?php echo $this->clientCrtl; ?>
                                </select>
                          </div>
                          <label for="department_name" class="col-sm-2 control-label">Department Name</label>
                          <div class="col-sm-3">
                              <!--<input type="text" class="form-control" id="department_name" name="department_name" value = "<?php echo $this->department_name;?>" placeholder="Department">-->
                                <select class="form-control select2" id="department_name" name="department_name" style = 'width:100%'>
                                </select>
                          </div>
                    </div>          
                    <div class="form-group">
                ...
                <div class="contract">
                    <div class="form-group">
                        <label for="contract_start_date" class="col-sm-2 control-label">Contract From</label>
                            <div class="col-sm-3">
                                <input type="text" class="form-control datepicker" id="contract_start_date" name="contract_start_date" value = "<?php echo format_date($this->contract_start_date);?>" placeholder="Date">
                            </div>
                        <label for="contract_end_date" class="col-sm-2 control-label">Contract End</label>
                            <div class="col-sm-3">
                                <input type="text" class="form-control datepicker" id="contract_end_date" name="contract_end_date" value = "<?php echo format_date($this->contract_end_date);?>" placeholder="Date">
                            </div> 
                    </div> 
                </div>
                <div class="fullncontract">
                    ...
                    <div class="form-group">
                        <label for="applicant_name" class="col-sm-2 control-label">Candidate Name</label>
                          <div class="col-sm-3">
                                <select class="form-control select2" id="applicant_name" name="applicant_name" style = 'width:100%'>
                                </select>
                          </div>  
                          <label for="job_title" class="col-sm-2 control-label">Job Title</label>
                          <div class="col-sm-3">
                                <select class="form-control select2" id="job_title" name="job_title" style = 'width:100%'>
                                </select>
                          </div>                         
                    </div>
                    <div class="form-group">
                        <label for="ot_hour" class="col-sm-2 control-label" >OT Hour</label>
                        <div class="col-sm-3">
                          <input type="text" class="form-control" id="ot_hour" name="ot_hour" value = "<?php echo $this->ot_hour;?>" placeholder="Hour eg:5">
                        </div>
                        <label for="total_working_day" class="col-sm-2 control-label">Total working day</label>
                          <div class="col-sm-3">
                            <input type="text" class="form-control" id="total_working_day" name="total_working_day" value = "<?php echo $this->total_working_day;?>">
                          </div>   
                    </div>                   
                    <div class="form-group">
                          <label for="payroll_person" class="col-sm-2 control-label">Payroll In charge Person</label>
                          <div class="col-sm-3">
                                 <select class="form-control select2" id="payroll_person" name="payroll_person" style = 'width:100%'>
                                     <?php echo $this->payrollEmplCrtl; ?>
                                 </select>
                          </div>            
                    </div>
                </div>
          ...
          <div class="row">
            <div class="col-xs-12" >
              <div class="box">
                ...
                <?php if(getWindowPermission($_SESSION['m'][$_SESSION['empl_id']],'create')){?>
                <button class="btn btn-primary pull-right" onclick = "window.location.href='timeshift.php?action=createForm'">Create New + </button>
                <?php }?>
                </div><!-- /.box-header -->
                    ...
                    ?>
                        <tr>
                            ...
                            <td class = "text-align-right">
                                ...
                                ?>
                                <button type="button" class="btn btn-primary btn-info " onclick = "location.href = 'timeshift.php?action=edit&job_id=<?php echo $row['job_id'];?>'">Edit</button>
                                <?php }?>
                                <?php 
                                if(getWindowPermission($_SESSION['m'][$_SESSION['empl_id']],'delete')){
                                ?>
                                <button type="button" class="btn btn-primary btn-danger " onclick = "confirmAlertHref('timeshift.php?action=delete&job_id=<?php echo $row['job_id'];?>','Confirm Delete?')">Delete</button>
                                <?php }?>
                            </td>
                        </tr>
                    <?php    
                    ...
                    success: function(data) {
                       ...
                       var table ="";
                        ...
                        if(jsonObj['aRemarks'] != null)
                         {
                              //table ="<a href='timeshift.php?action=edit&current_tab=followup&applicant_id=" + jsonObj['aRemarks']['applicant_id'][0] + "'><button type='button' class='btn btn-info' style= 'float:right'>Add Remarks</button></a><br><br>";
...
//                    console.log(jsonObj['assign_time']);
                    ...
                    table = table + "<tr><td>" + n + "</td><td>" + jsonObj['aRemarks']['empl_name'][i] +"</td><td>" + jsonObj['aRemarks']['assign_to'][i] + "</td><td>" + jsonObj['aRemarks']['interview_company'][i] + "</td><td>" + jsonObj['aRemarks']['follow_type'][i] + "</td><td>" + jsonObj['aRemarks']['received_offer'][i] + "</td><td>" + "<a href='timeshift.php?action=edit&current_tab=followup&applicant_id=" + jsonObj['aRemarks']['applicant_id'][i] + "&follow_id=" + jsonObj['aRemarks']['follow_id'][i] + "'>" + jsonObj['aRemarks']['comments'][i] + "</a></td><td>" + jsonObj['aRemarks']['time'][i] + "</td><td>" + jsonObj['aRemarks']['date'][i] + "</td></tr>" 
                    n++;
                    }
...
//    mail("jasonchong@firstcom.com.sg", $subject, $message, $headers);
    ...
    public function saveNotification(){
        ...
        $table_field = array('noti_id','noti_to','noti_url','noti_parent_id','noti_desc','noti_view_status');
            ...
            $table_value = array('', $assign_id, 'timeshift.php?action=edit&job_id='.$this->job_id,$this->job_id ,$notification_desc['4'] ,0);   
...
<!--        <div class="form-group">
    ...
    public function getDepartment(){
        ...
        $i = 0;
        while($row = mysql_fetch_array($query)){
//            $data['timeshift_department'][$i] = $row['timeshift_department'];
            $id = $row['timeshift_id'];
            $code = $row['timeshift_department'];
            $data[$i] ="<option value = '$id'>$code</option>"; 
        $i++;
        }
        return $data;
    }
    ...
    public function getApplicant(){
        $this->client_id = escape($_REQUEST['client_id']);
        $sql = "SELECT a.applicant_id, a.applicant_name FROM db_followup f INNER JOIN db_applicant a ON f.applfollow_id = a.applicant_id WHERE interview_company = '$this->client_id' and follow_type = '0'";
        $query = mysql_query($sql);
        $i = 0;
        while($row = mysql_fetch_array($query)){
            $id = $row['applicant_id'];
