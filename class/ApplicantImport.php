    ...
    public function showImportDetail(){
        ...
        $importNumber = 1;
            while($row = mysql_fetch_array($query)){
               if($row['file_type'] == "docx" || $row['file_type'] == "txt" || $row['file_type'] == "pdf"){             
                ?>
        ...
        $fileFormat = strtolower(pathinfo($row['file_name'],PATHINFO_EXTENSION));
                if($row['file_type'] == "docx"){
                   ...
                   $filterWords = str_replace(" : ", " ", $filterWords);
                    ...
                    unlink("dist/import/$deletefile");
                    $sql = "DELETE FROM db_importfile WHERE file_name ='$deletefile'";
                    mysql_query($sql);
                }
                ...
                for($x = 0; $x < $convertlength; $x++) {
                  if ($convert[$x] == "Name" && $Noname == 0){
                      ...
                      ?><label for="import_email<?php echo $importNumber ?>" class="col-sm-2 control-label">Email </label>
                        <div class="col-sm-2">
                        <input type="text" class="form-control " id="import_email<?php echo $importNumber ?>" name="import_email[<?php echo $row['file_name'] ?>]" value = "<?php echo strtolower($convert[$x+1]); ?>" placeholder="Email">
                        </div>
                      <?php $Noemail++;
                  }
             ...
             $importNumber++;
                    ...
                    //delete other file
                    $deletefile = $row['file_name'];
                    $deletetype = $row['file_type'];
                    unlink("dist/import/$deletefile");
                    $sql = "DELETE FROM db_importfile WHERE file_name ='$deletefile' AND file_type = '$deletetype'";
                    mysql_query($sql);
                }
                  }?>
...
<?php
    ...
    public function saveImportData(){
        ...
        $ftable_field = array('assign_to','follow_type','interview_time','interview_date',
                              ...
                              'fol_offer_salary','attend_interview','received_offer','comments',
        ...
        for($i=0;$i<sizeof($_POST['import_name']);$i++){
            ...
            $remark = "Save Import Followup Data";
                ...
                if($_SESSION['empl_group'] == "4") {
                    ...
                    $ftable_value = array($_POST['import_assign'][$file_name], '3', '', '', '', '', '', '', '', '', '', '', $_POST['import_comments'][$file_name], 0, $this->applicant_id);
                    ...
                    $ftable_value = array('', '1', '', '', '', '', '', '', '', '', '', '', $_POST['import_comments'][$file_name], 0, $this->applicant_id);    
                    $this->save->SaveData($ftable_field,$ftable_value,'db_followup','follow_id',$remark);
                    $this->follow_id = $this->save->lastInsert_id;
