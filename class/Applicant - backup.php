...
<!--              <button type="button" class="btn btn-primary btn-info " onclick = "confirmAlertHref('applicant.php?action=saveFamily&applicant_id=<?php echo $this->applicant_id;?>&appfamily_name=<?php echo $this->appfamily_name;?>','Confirm Save?')">Save</button>-->
        ...
        <table id="followup_table" class="table table-bordered table-hover dataTable">
                    ...
                    <tbody>
                    <?php   
                      $sql = "select f.follow_id, f.follow_type, f.assign_to, left(f.insertDateTime,10) as date, right(f.insertDateTime, 8) as time, f.interview_company, f.attend_interview, f.received_offer, f.comments, f.insertBy, e.empl_name from db_followup f inner join db_empl e on f.insertBy = e.empl_id where f.applfollow_id = '$this->applicant_id' ORDER BY YEAR(date) DESC, MONTH(date) DESC, DAY(date) DESC, time DESC";
                      $query = mysql_query($sql);
                      $i = 1;
                      while($row = mysql_fetch_array($query)){
                    ?>
...
//           $this->applicantsalary_amount = 0;
    ...
    public function getUploadFileForm(){
        ...
        <div class="box-body table-responsive">     
                ...
                <div class="col-sm-4">
                  <button type = "button" class="btn btn-info upload_file_btn" onclick = "'applicant.php?action=edit&current_tab=UploadFile&applicant_id=<?php echo $this->applicant_id;?>'">Save</button>
                </div>
...
<!--                          <li tab = "Resume File" class="tab_header <?php if($this->current_tab == "Resume File"){ echo 'active';}?>" ><a href="#resumefile" data-toggle="tab">Upload Resume</a></li>-->
...
//                                 echo file_get_contents(include_webroot . "dist/qrcode/?data=$this->applicant_code");
                          ...
                          <div class=" tab-pane <?php if($this->current_tab == "Resume File"){ echo 'active';}?>" id="resumefile">
...
//                           window.location.href = url + "&current_tab=" + $('#current_tab').val();
            ...
            $('.save_followup_btn').click(function(){
                var data = "action=saveFollowup&applicant_id=<?php echo $this->applicant_id;?>&assign_to="+$('#assign_to').val()+"&follow_type="+$('#follow_type').val()+"&interview_time="+$('#interview_time').val();
                    ...
                    data = data + "&applicant_attend="+$('#applicant_attend').val()+"&received_offer="+$('#received_offer').val()+"&follow_notice="+$('#follow_notice').val()+"&followup_comments="+$('#followup_comments').val();
                $.ajax({ 
                    type: 'POST',
                    url: 'applicant.php',
                    cache: false,
                    ...
                    success: function(data) {
                       var jsonObj = eval ("(" + data + ")");
                       //if(jsonObj.status == 1){
                           var url = '<?php echo $_SERVER['PHP_SELF'] . "?action=saveFollowup&applicant_id={$_REQUEST['applicant_id']}";?>';
                           window.location.href = url + "&current_tab=followup" ;
                       //}else{
                           //alert("Fail to add line.");
                       //}
                       issend = false;
      ...
      <!-- Full Width Column -->
        ...
        <section class="content-header">
          ...
          <section class="content">
            <div class="box box-success">
              <div class="box-header with-border">
                <h3 class="box-title"><?php if($this->applicant_id > 0){ echo "Update Candidate";}else{ echo "Create New Applicant";}?></h3>
                <button type = "button" class="btn btn-primary pull-right" style = '' onclick = "window.location.href='applicant.php'">Search</button>
                <?php if(getWindowPermission($_SESSION['m'][$_SESSION['empl_id']],'create')){?>
                <button type = "button" class="btn btn-primary pull-right" style = 'margin-right:10px;' onclick = "window.location.href='applicant.php?action=createForm'">Create New</button>
                <?php }?>
...
//                console.log($('#follow_type').val());
            ...
            $('.save_experience_btn').click(function(){
                var data = "action=saveWorkExperience&applicant_id=<?php echo $this->applicant_id;?>&previous_company="+$('#previous_company').val()+"&previous_position="+$('#previous_position').val();
                    data = data + "&previous_salary="+$('#previous_salary').val()+"&reason_leave="+$('#reason_leave').val()+"&exp_id="+$('#exp').val()+"&duration_conform="+$('#duration_conform').val();
...
<!--                          <div class=" tab-pane <?php if($this->current_tab == "Foreign Worker"){ echo 'active';}?>" id="foreign_worker">
                  ...
                  <div class="box-footer">
                    ...
                    ?>
                    <button type = "submit" class="btn btn-info">Submit</button>
                    <button type="button" class="btn btn-primary btn-warning " target="_blank" style="margin-left:10px" onclick = "confirmAlertHref('applicant.php?action=printPDF&applicant_id=<?php echo $this->applicant_id;?>','Confirm')">Print</button>
                    <?php }?>
                  </div><!-- /.box-footer -->
                </form>
            </div><!-- /.box -->
...
//                    console.log(jsonObj['assign_time']);
                    ...
                    table = table + "<tr><td>" + n + "</td><td>" + jsonObj['aRemarks']['empl_name'][i] +"</td><td>" + jsonObj['aRemarks']['assign_to'][i] + "</td><td>" + jsonObj['aRemarks']['interview_company'][i] + "</td><td>" + jsonObj['aRemarks']['follow_type'][i] + "</td><td>" + jsonObj['aRemarks']['received_offer'][i] + "</td><td>" + "<a href='applicant.php?action=edit&current_tab=followup&applicant_id=" + jsonObj['aRemarks']['applicant_id'][i] + "&follow_id=" + jsonObj['aRemarks']['follow_id'][i] + "'>" + jsonObj['aRemarks']['comments'][i] + "</a></td><td>" + jsonObj['aRemarks']['time'][i] + "</td><td>" + jsonObj['aRemarks']['date'][i] + "</td></tr>" 
                    n++;
                    }
