<?php
    ...
    $o->payroll_client = escape($_POST['payroll_client']);
        ...
        case "create":
            if(getWindowPermission($o->menu_id,'create')){
                if($o->create()){
                    ...
                    rediectUrl("applicantpayroll.php?action=edit&payroll_id=$o->payroll_id",getSystemMsg(1,'Create data successfully'));
                }else{
                    $_SESSION['status_alert'] = 'alert-error';
                    $_SESSION['status_msg'] = "Create fail.";
                    rediectUrl("applicantpayroll.php",getSystemMsg(0,'Create data fail'));
                }
            }
            exit();
            break;
        case "update":
            if(getWindowPermission($o->menu_id,'update')){
                if($o->update()){
                    ...
                    rediectUrl("applicantpayroll.php?action=edit&payroll_id=$o->payroll_id",getSystemMsg(1,'Update data successfully'));
                }else{
                    $_SESSION['status_alert'] = 'alert-error';
                    $_SESSION['status_msg'] = "Update fail.";
                    rediectUrl("applicantpayroll.php?action=edit&payroll_id=$o->payroll_id",getSystemMsg(0,'Update data fail'));
                }
            }
            exit();
            break;  
        case "edit":
            if(getWindowPermission($o->menu_id,'update')){
                if(!$o->fetchPayrollDetail(" AND payroll_id = '$o->payroll_id'","","",1)){
                    rediectUrl("applicantpayroll.php",getSystemMsg(0,'Fetch Data fail'));
                    exit();
                }else{
        ...
        case "delete":
            if(getWindowPermission($o->menu_id,'delete')){
                if($o->delete()){
                    ...
                    rediectUrl("applicantpayroll.php",getSystemMsg(1,'Delete data successfully'));
                }else{
                    $_SESSION['status_alert'] = 'alert-error';
                    $_SESSION['status_msg'] = "Delete fail.";
                    rediectUrl("applicantpayroll.php",getSystemMsg(0,'Delete data fail'));
                }
            }
            exit();
            break;   
...
//            }else{
//                rediectUrl("applicantpayroll.php",getSystemMsg(0,'Password verification fail.'));
//            }
            exit();
            break; 
        case "updateSelfView":
        ...
        case "varify_password_bypass":
            if($o->empl_id <= 0){
                rediectUrl("applicantpayroll.php",getSystemMsg(0,'Employee data not found.'));
                exit();
            }
            ...
            if($o->varify_password == ""){
                rediectUrl("applicantpayroll.php",getSystemMsg(0,'Password cannot be empty.'));
                exit();
            }
...
//            }
        ...
        case "getDepartment":
            ...
            echo json_encode(array('department'=>$department_array));
            exit();
            break; 
